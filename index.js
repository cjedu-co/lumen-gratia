...
// server/routes.ts
var imageUploadDir = path.join(process.cwd(), "public/uploads");
...
async function registerRoutes(app2) {
  ...
  app2.get("/api/courses", async (req, res) => {
    try {
      ...
      const academyType = validateAcademyType(req.query.academyType);
      if (category) {
        const courses3 = await storage.getCoursesByCategory(category, academyType);
        return res.json(courses3);
      }
      const courses2 = await storage.getCourses(academyType);
      res.json(courses2);
    } catch (error) {
  ...
  app2.get("/api/news", async (req, res) => {
    try {
      const academyType = validateAcademyType(req.query.academyType);
      const news2 = await storage.getNews(academyType);
      res.json(news2);
    } catch (error) {
  ...
  app2.get("/api/gallery", async (req, res) => {
    try {
      const academyType = validateAcademyType(req.query.academyType);
      const gallery2 = await storage.getGallery(academyType);
      res.json(gallery2);
    } catch (error) {
  ...
  app2.post("/api/anonymous-posts", async (req, res) => {
    try {
      const validatedData = insertAnonymousPostSchema.parse(req.body);
      const post = await storage.createAnonymousPost(validatedData);
      res.status(201).json({
        message: "Anonymous post submitted successfully!",
        postId: post.id
      });
    } catch (error) {
      if (error instanceof z.ZodError) {
      ...
      res.status(500).json({ message: "Failed to submit anonymous post" });
    }
  });
  app2.get("/api/anonymous-posts", isAuthenticated, async (req, res) => {
    try {
      const posts = await storage.getAnonymousPosts();
      res.json(posts);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch anonymous posts" });
    }
  });
  app2.get("/api/anonymous-posts/:id", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid post ID" });
      }
      const post = await storage.getAnonymousPostById(id);
      if (!post) {
        return res.status(404).json({ message: "Anonymous post not found" });
      }
      res.json(post);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch anonymous post" });
    }
  });
  app2.put("/api/anonymous-posts/:id/read", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid post ID" });
      }
      const success = await storage.markAnonymousPostAsRead(id);
      if (!success) {
        return res.status(404).json({ message: "Anonymous post not found" });
      }
      res.json({ message: "Anonymous post marked as read" });
    } catch (error) {
      res.status(500).json({ message: "Failed to mark anonymous post as read" });
    }
  });
  app2.delete("/api/anonymous-posts/:id", async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid post ID" });
      ...
      if (req.isAuthenticated()) {
        const success2 = await storage.deleteAnonymousPost(id);
        if (!success2) {
          return res.status(404).json({ message: "Anonymous post not found" });
        }
        return res.json({ message: "Anonymous post deleted successfully" });
      }
      const { password } = req.body;
      if (!password) {
        return res.status(400).json({ message: "Password required to delete this post" });
      }
      const success = await storage.deleteAnonymousPost(id, password);
      if (!success) {
        return res.status(403).json({ message: "Invalid password or post not found" });
      }
      res.json({ message: "Anonymous post deleted successfully" });
    } catch (error) {
      res.status(500).json({ message: "Failed to delete anonymous post" });
    }
  });
  app2.get("/api/blog", async (req, res) => {
    try {
      ...
      const posts = await storage.getBlogPosts(academyType, limit);
      res.json(posts);
    } catch (error) {
      console.error("Error fetching blog posts:", error);
  ...
  app2.get("/api/blog/featured", async (req, res) => {
    try {
  ...
  app2.get("/api/blog/tags/:tag", async (req, res) => {
    try {
      ...
      const limit = req.query.limit ? parseInt(req.query.limit) : void 0;
      const posts = await storage.getBlogPostsByTag(tag, limit);
      res.json(posts);
    } catch (error) {
      console.error(`Error fetching posts with tag ${req.params.tag}:`, error);
  ...
  app2.put("/api/blog/:id", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid ID format" });
  ...
  app2.delete("/api/blog/:id", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid ID format" });
  ...
  app2.post("/api/change-password", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!oldPassword || !newPassword) {
        return res.status(400).json({ error: "\uAE30\uC874 \uBE44\uBC00\uBC88\uD638\uC640 \uC0C8 \uBE44\uBC00\uBC88\uD638\uAC00 \uD544\uC694\uD569\uB2C8\uB2E4" });
      }
      if (newPassword.length < 4) {
        return res.status(400).json({ error: "\uC0C8 \uBE44\uBC00\uBC88\uD638\uB294 \uCD5C\uC18C 4\uC790 \uC774\uC0C1\uC774\uC5B4\uC57C \uD569\uB2C8\uB2E4" });
      }
      const success = await storage.changePassword(user.username, oldPassword, newPassword);
      if (!success) {
        return res.status(400).json({ error: "\uAE30\uC874 \uBE44\uBC00\uBC88\uD638\uAC00 \uC62C\uBC14\uB974\uC9C0 \uC54A\uC2B5\uB2C8\uB2E4" });
      }
  ...
  app2.post("/api/upload", isAuthenticated, imageUpload.single("image"), async (req, res) => {
    try {
      if (!req.file) {
  ...
  app2.get("/api/gallery", async (req, res) => {
    try {
      const academyType = validateAcademyType(req.query.academyType);
      const galleryItems = await storage.getGallery(academyType);
  ...
  app2.post("/api/gallery", isAuthenticated, async (req, res) => {
    try {
      ...
      const gallery2 = await storage.createGallery(result.data);
      res.status(201).json(gallery2);
    } catch (error) {
      console.error("Error creating gallery item:", error);
      res.status(500).json({ message: "Failed to create gallery item" });
    }
  });
  app2.put("/api/gallery/:id", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
  ...
  app2.delete("/api/gallery/:id", isAuthenticated, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      if (isNaN(id)) {
        return res.status(400).json({ message: "Invalid gallery ID" });
...
app.use("/assets", express2.static(path4.join(process.cwd(), "public/assets")));
